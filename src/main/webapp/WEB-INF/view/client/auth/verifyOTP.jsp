...
<%--        <div class="col-md-6"> &lt;%&ndash; Tăng độ rộng một chút để chứa thông báo và nút &ndash;%&gt;--%>
...
<%--                        &lt;%&ndash; Kiểm tra message chứa "không đúng" để dùng alert-danger, còn lại dùng alert-info &ndash;%&gt;--%>
<%--                        <c:choose>--%>
<%--                            <c:when test="${message.contains('không đúng') or message.contains('lỗi xảy ra')}">--%>
...
<%--                    <p class="text-center">Một mã OTP đã được gửi đến địa chỉ email: <strong>${email}</strong>. Vui lòng kiểm tra hộp thư của bạn.</p>--%>
<%--                    &lt;%&ndash; Controller sẽ đảm bảo ${email} có sẵn từ flashAttribute hoặc được thêm vào model khi render lại &ndash;%&gt;--%>
...
<%--                            &lt;%&ndash; name="OTP" khớp với @RequestParam(name = "OTP") trong controller.--%>
<%--                                 Không cần required ở đây vì có thể người dùng nhấn "Gửi lại OTP" &ndash;%&gt;--%>
<%--                        </div>--%>

<%--                        <div class="form-actions mb-3">--%>
<%--                            <button type="submit" name="action" value="VERIFY-OTP" class="btn btn-primary">Xác nhận OTP</button>--%>
<%--                            <button type="submit" name="action" value="RESENT-OTP" class="btn btn-secondary" formnovalidate>Gửi lại OTP</button>--%>
<%--                            &lt;%&ndash; formnovalidate cho nút gửi lại để không check required của OTP input &ndash;%&gt;--%>
<%--                        </div>--%>
<%--                        <div class="text-center">--%>
<%--                            <a href="<c:url value='/forgotPassword'/>">Quay lại nhập email</a>--%>
<%--                        </div>--%>
<%--                    </form>--%>
<%--                </div>--%>
<%--            </div>--%>
...
<%--                    <c:if test="${not empty message}">--%>
<%--                        <c:choose>--%>
<%--                            <c:when test="${message.contains('không đúng') or message.contains('lỗi xảy ra') or message.toLowerCase().contains('error')}">--%>
<%--                                <div class="alert alert-danger my-2" role="alert">--%>
<%--                                        ${message}--%>
<%--                                </div>--%>
<%--                            </c:when>--%>
...
<%--                        <div class="form-actions mb-3">--%>
<%--                            <button type="submit" name="action" value="VERIFY-OTP" class="btn btn-primary">Xác nhận OTP</button>--%>
<%--                            <button type="submit" id="resendOtpBtn" name="action" value="RESENT-OTP" class="btn btn-secondary" formnovalidate>--%>
            ...
            <div class="card shadow-lg">
                ...
                <div class="card-body">
                    ...
                    <form id="verifyOtpForm" action="<c:url value='/verifyOtp'/>" method="post">
                        ...
                        <div class="mb-3">
                            <label for="otp" class="form-label">Nhập mã OTP:</label>
                            <input type="text" class="form-control" id="otp" name="OTP" placeholder="123456">
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function startTimer() {
            console.log("startTimer called. Initial countdownTime:", countdownTime); // DEBUG
            resendButton.disabled = true;
            countdownTimerSpan.textContent = formatTime(countdownTime);
            console.log("Initial time displayed:", countdownTimerSpan.textContent); // DEBUG
