...
<head>
  ...
  <style>
    ...
    .otp-input-group input { text-align: center; font-size: 1.5rem; letter-spacing: 0.5rem; border-radius: 0.375rem !important; }
    .form-actions { display: flex; gap: 0.75rem; }
    .form-actions .btn { flex: 1; }
    #resendOtpBtn { font-weight: 500; }
    #countdownTimer { font-weight: 500; color: var(--bs-primary); }
  </style>
</head>
...
<div class="auth-wrapper">
  <div class="card auth-card" style="max-width: 500px">
    <div class="card-header">
      <a href="<c:url value='/'/>" class="logo-text-auth"><spring:message code="page.verifyOtp.header.brandName"/></a>
      <h3 class="font-weight-light my-1"><spring:message code="page.verifyOtp.header.title"/></h3>
    </div>
    <div class="card-body">
      ...
      <c:if test="${not empty messageSuccess and not messageSuccess.contains('Xác thực mã OTP thành công')}">
        <div class="alert alert-info alert-auth my-3" role="alert">
            ${messageSuccess}
        </div>
      </c:if>

      <form id="verifyOtpForm" action="<c:url value='/verifyOtp'/>" method="post">
        <input type="hidden" name="email" value="${email}" />
        <security:csrfInput />
        <div class="form-floating mb-3 otp-input-group">
          <spring:message code="page.verifyOtp.form.placeholder.otp" var="otpPlaceholder"/>
          <spring:message code="page.verifyOtp.form.title.otpValidation" var="otpTitleValidation"/>
          <input type="text" class="form-control" id="otp" name="OTP" placeholder="${otpPlaceholder}" maxlength="6" pattern="[A-Z0-9]{6}" title="${otpTitleValidation}" required/>
          <label for="otp"><spring:message code="page.verifyOtp.form.label.otp"/></label>
        </div>
        <div class="form-actions mb-3">
          <button type="submit" name="action" value="VERIFY-OTP" class="btn btn-primary btn-lg">
            <spring:message code="page.verifyOtp.form.button.verify"/>
          </button>
          <button type="submit" id="resendOtpBtn" name="action" value="RESENT-OTP" class="btn btn-outline-secondary btn-lg" formnovalidate>
            <spring:message code="page.verifyOtp.form.button.resend"/> <span id="countdownTimer"></span>
          </button>
        </div>
      </form>
      <div class="text-center mt-3">
        <a href="<c:url value='/forgotPassword'/>" class="small text-muted text-decoration-none">
          <i class="fas fa-arrow-left me-1"></i> <spring:message code="page.verifyOtp.link.backToEmailInput"/>
        </a>
      </div>
    </div>
    <div class="card-footer">
      <div class="small">
        ...
        <a href="<c:url value='/contact-us'/>" class="text-primary fw-medium"><spring:message code="page.verifyOtp.footer.contactUs"/></a>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Bắt đầu timer khi trang được tải lần đầu (nếu không phải là gửi lại)
    // hoặc nếu không có thông báo nào đặc biệt
